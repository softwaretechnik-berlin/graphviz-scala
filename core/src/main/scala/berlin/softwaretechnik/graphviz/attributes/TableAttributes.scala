...
trait Columns
object Columns {
  case object Star extends Columns {
...
case class TableAttributes(
   ...
   */
  cellBorder: java.lang.Integer = null,
  ...
  /**
   * provides general formatting information concerning the columns. At present,
   * the only legal value is "*", which causes a vertical rule to appear between
   * every cell in every row.
   */
